...
program mknatorb
    ...
    print '(/20x,a/20x,a/)', 'PROGRAM MKNATORB    28 DEC 2019', '==============================='
    ...
    !allocate(cno(nsym,n_natorb*max(nbas(1),nbas(2))))
    ...
    do ino = 1, n_natorb
        ...
        do iact = 1, n_act
            ...
            cno(isym, m * (ino_offs(isym) - 1) + 1:m * ino_offs(isym)) = &
                    cno(isym, m * (ino_offs(isym) - 1) + 1:m * ino_offs(isym)) + &
                            w * cmo(isym, m * (i - 1) + 1:m * i)
        end do
...
subroutine read_natorb(filename, n_act, act_eps, act_nsym, n_natorb, occ_numbers, no_nsym, natorb_coef)
    ...
    n_gerade = count(no_nsym == 1)
    n_ungerade = count(no_nsym == 2)
    ...
    do i = 1, n_act
        print '(i3,f16.8,i5)', i, act_eps(i), act_nsym(i)
    end do
    print *, 'OCC NUMBERS AND SYMMETRIES OF NATURAL SPINORS:'
    do i = 1, n_natorb
        print '(i3,f16.8,i5)', i, occ_numbers(i), no_nsym(i)
    end do
...
subroutine read_dfpcmo(filename, nsym, idim, scf_energy, cmo, eps, ibeig)
    ...
    read (fd, *) (real_buf(i), i = 1, ncoef(1) + ncoef(2))
