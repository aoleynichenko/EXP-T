...
int main(int argc, char **argv)
{
    ...
    else if (target == TARGET_TURBOMOLE) {
        gen_turbomole(stdout, mol, bas, ecp);
    }
    else if (target == TARGET_ONEPROP) {
        gen_oneprop(stdout, mol, bas, ecp);
    }
...
void parse_argv(int argc, char **argv, char *file_name, int maxlen, int *target)
{
    if (argc == 2) {
        ...
        *target = TARGET_DIRAC;
        ...
        else if (strcmp(argv[1], "--turbomole") == 0) {
            *target = TARGET_TURBOMOLE;
        }
        else if (strcmp(argv[1], "--oneprop") == 0) {
            *target = TARGET_ONEPROP;
        }
        else {
            print_usage(argv[0]);
            exit(1);
