...
void tm2c_interface(char *moints_file_1, char *moints_file_2, char *moints_file_prop, cc_options_t *opts)
{
    ...
    int spinor_irreps[CC_MAX_SPINORS];
    ...
    size_t hcore_nbytes = sizeof(double complex) * num_spinors * num_spinors;
    ...
    while (fscanf(hcore_file, "%d%d%lf%lf", &i1, &i2, &h_re, &h_im) == 4) {
        hcore[i1 * num_spinors + i2] = h_re + h_im * I;
        hcore[i2 * num_spinors + i1] = h_re - h_im * I;
    }
    ...
    setup_symmetry(CC_GROUP_COMPLEX, "C1", nsym, rep_names, irrep_a1, &mult_table);
    ...
    create_spinor_info(num_spinors, spinor_irreps, orb_energies, occ_numbers);

    create_spinor_blocks(tile_size);
    setup_occupation_numbers(opts, num_spinors, spinor_info);
