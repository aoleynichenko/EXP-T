...
int sector_0h1p(cc_options_t *opts)
{
    ...
    calculate_properties_finite_order_method(0, 1);

    /*
     * analytic evaluation of density matrices, expectation values
     * of property operators, transition properties
     */
    if (cc_opts->calc_density[0][1] == CC_DENSITY_MATRIX_LAMBDA) {
        if (cc_opts->density_num_states[0][1] == -1) {
            finite_order_density_matrix_all_states(0, 1);
        }
        else {
            for (int i = 0; i < cc_opts->density_num_states[0][1]; i++) {
                cc_denmat_query_t *query = cc_opts->density_target_states[0][1] + i;
                finite_order_density_matrix(0, 1, query->rep1_name, query->state1, query->rep2_name, query->state2);
            }
        }
    }

    /*
     * lambda equations & analytic density matrices & properties
     */
    /*if (cc_opts->calc_lambda_0h1p) {
        for (int i = 0; i < cc_opts->lambda_0h1p_num_states; i++) {
            cc_denmat_query_t *query = cc_opts->lambda_0h1p_states + i;

            sector_0h1p_lambda_equations(query->rep1_name, query->state1);
            sector_0h0p_lambda_equations();
...
void construct_doubles_0h1p()
{
    tmplt("s2nw", "phpp", "1000", "1234", NOT_PERM_UNIQUE);
