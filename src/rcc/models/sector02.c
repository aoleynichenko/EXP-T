...
int finite_order_density_matrix_0h2p(char *irrep_name, int state_number);
...
int sector_0h2p(cc_options_t *opts)
{
     ...
     */
    calculate_wavefunction_norms_and_overlaps(0, 2);
    calculate_model_space_natural_spinors(0, 2);
    calculate_model_space_properties(0, 2);
    calculate_properties_finite_order_method(0, 2);

    /*
     * finite-order calculation of density matrices
     */
    if (cc_opts->calc_density[0][2] == CC_DENSITY_MATRIX_LAMBDA) {
        if (cc_opts->density_num_states[0][2] == -1) {
            finite_order_density_matrix_all_states(0, 2);
        }
        else {
            for (int i = 0; i < cc_opts->density_num_states[0][2]; i++) {
                cc_denmat_query_t *query = cc_opts->density_target_states[0][2] + i;
                finite_order_density_matrix(0, 2, query->rep1_name, query->state1, query->rep2_name, query->state2);
            }
        }
    }
