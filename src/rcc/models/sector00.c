...
int sector_0h0p(cc_options_t *opts)
{
    ...
    #ifdef TENSOR_TRAIN
    if (cc_opts->tt_options.tt_module_enabled) {
        tt_ccsd();
        return EXIT_SUCCESS;
    }
    #endif // TENSOR_TRAIN
...
//    tensor_decomposition_benchmark("t3c", 1e-8);
    ...
    else if (cc_opts->calc_density[0][0] == CC_DENSITY_MATRIX_EXPECTATION) {
...
void sort_integrals_0h0p()
{
    ...
    if (cc_opts->calc_density[0][0] == CC_DENSITY_MATRIX_LAMBDA || cc_opts->calc_lambda_0h1p) {
