...
#include "options.h"
#include "sort.h"
#include "../properties/finite_order_dm.h"
...
int sector_0h3p(cc_options_t *opts)
{
    ...
    if (cc_opts->cc_model < CC_MODEL_CCSDT_2) {
        ...
        // try to read effective Hamiltonian from disk
        int calc_veff = 1;
        if (cc_opts->reuse_amplitudes[0][3]) {
            printf("\n Trying to read effective hamiltonian (sector 0h3p) from disk ...\n");
            if (diagram_read_binary("veff03.dg") != NULL) {
                printf(" Heff{0h3p} diagram successfully read from disk\n");
                calc_veff = 0;
            }
            else {
                printf(" Heff{0h3p} diagram will be calculated\n");
            }
            printf("\n");
        }

        // otherwise re-calculate it non-iteratively
        if (calc_veff) {
            heff03_ccsd();
            ...
            save_cluster_amplitudes(0, 3, NULL, NULL, NULL, "veff03");
        }
    }
     ...
     */
    calculate_wavefunction_norms_and_overlaps(0, 3);
    calculate_model_space_natural_spinors(0, 3);
    calculate_model_space_properties(0, 3);
    calculate_properties_finite_order_method(0, 3);

    /*
     * construct finite-order density matrices if needed
     */
    if (cc_opts->calc_density[0][3] == CC_DENSITY_MATRIX_LAMBDA) {
        if (cc_opts->density_num_states[0][3] == -1) {
            finite_order_density_matrix_all_states(0, 3);
        }
        else {
            for (int i = 0; i < cc_opts->density_num_states[0][3]; i++) {
                cc_denmat_query_t *query = cc_opts->density_target_states[0][3] + i;
                finite_order_density_matrix(0, 3, query->rep1_name, query->state1, query->rep2_name, query->state2);
            }
        }
    }
...
void init_amplitudes_0h3p()
{
    ...
    if (cc_opts->reuse_amplitudes[0][3]) {
        ...
        if (diagram_read_binary("z3c.dg") != NULL) {
