...
int sector_2h0p(cc_options_t *opts)
{
     ...
     */
    // old:
    //calculate_model_space_properties(2, 0);
    //calculate_model_space_natural_spinors(2, 0);

    calculate_wavefunction_norms_and_overlaps(2, 0);
    calculate_model_space_natural_spinors(2, 0);
    calculate_model_space_properties(2, 0);
    calculate_properties_finite_order_method(2, 0);
...
void init_amplitudes_2h0p()
{
    ...
    if (cc_opts->reuse_amplitudes[2][0]) {
        ...
        if (diagram_read_binary("g2c.dg") != NULL) {
        ...
        if (diagram_read_binary("veff20.dg") != NULL) {
            printf(" Heff{2h0p} diagram successfully read from disk\n");
            calc_veff = 0;
        }
        else {
            printf(" Heff{2h0p} diagram will be calculated\n");
        }
        if (triples) {
            if (diagram_read_binary("g3c.dg") != NULL) {
