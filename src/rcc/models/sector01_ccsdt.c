 ...
 * This file containing code implementing the CCSDT model in the 0h1p sector.
 *
 ...
 * Relativistic Fock Space Coupled Cluster Method for Many-Electron Systems:
...
void sector_0h1p_ccsd_t()
{
    dg_stack_pos_t pos;

    timer_new_entry("01-CCSD(T)", "0h1p -- Triples pert correction +T(3)");
    timer_start("01-CCSD(T)");

    printf("\n  FS-CCSD(T) perturbative correction\n");
    printf(" ------------------------------------\n");
    ...
    /*
     * calculate (T) contribution to the effective interaction (V_eff)
     */
...
void sector_0h1p_ccsd_t4()
{
    ...
    t3_0h1p_const_contrib_to_triples(PT_2);
    construct_triples_0h1p(PT_2);
    t3_contrib_to_folded_0h1p(PT_2);  // however, folded are PT3+
    rename_diagram("s3nw", "s3c");
    diveps("s3c");
    copy("s3c", "s3c_pt2");
    double t2 = abs_time();
    ...
    t3_contrib_to_singles_0h1p(PT_3);
    t3_contrib_to_doubles_0h1p(PT_3);
    pos = get_stack_pos();
    closed("s1nw", "x");
    restore_stack_pos(pos);
    ...
    construct_triples_0h1p(PT_3);
    t3_contrib_to_folded_0h1p(PT_3);
    copy("s3nw", "s3c");
    diveps("s3c");
    double t5 = abs_time();
    printf("     0h1p %.3f sec\n", t5 - t4);
