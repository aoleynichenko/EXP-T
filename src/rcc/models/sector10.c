...
int sector_1h0p(cc_options_t *opts)
{
     ...
     */
    calculate_wavefunction_norms_and_overlaps(1, 0);
    calculate_model_space_natural_spinors(1, 0);
    calculate_model_space_properties(1, 0);
    calculate_properties_finite_order_method(1, 0);

    /*
     * analytic evaluation of density matrices, expectation values
     * of property operators, transition properties
     */
    if (cc_opts->calc_density[1][0] == CC_DENSITY_MATRIX_LAMBDA) {
        if (cc_opts->density_num_states[1][0] == -1) {
            finite_order_density_matrix_all_states(1, 0);
        }
        else {
            for (int i = 0; i < cc_opts->density_num_states[1][0]; i++) {
                cc_denmat_query_t *query = cc_opts->density_target_states[1][0] + i;
                finite_order_density_matrix(1, 0, query->rep1_name, query->state1, query->rep2_name, query->state2);
            }
        }
    }
...
void init_amplitudes_1h0p()
{
    ...
    if (cc_opts->reuse_amplitudes[1][0]) {
        ...
        // Singles
        if (diagram_read_binary("h1c.dg") != NULL) {
        ...
        // Doubles
        if (diagram_read_binary("h2c.dg") != NULL) {
        ...
        if (diagram_read_binary("veff10.dg") != NULL) {
        ...
        // maybe Triples
        if (triples) {
            if (diagram_read_binary("h3c.dg") != NULL) {
