...
)
{
    ...
    int iter;
    static double prev_ecorr = 0.0; // correlation energy from the previous iteration
    ...
    for (iter = 1; iter <= cc_opts->maxiter; iter++) {
        ...
        /*
         * specific for CCSD in the 0h0p sector:
         * print correlation energy at each iteration for the 'high' print level
         */
        if (cc_opts->print_level >= CC_PRINT_HIGH &&
            cc_opts->sector_h == 0 && cc_opts->sector_p == 0 &&
            cc_opts->curr_in_lambda_equations == 0) {
            double ecorr = cc_energy();
            printf("\n");
            printf(" e(corr), a.u.       = %25.16f\n", ecorr);
            printf(" delta e(corr), a.u. = %25.16f\n", ecorr - prev_ecorr);
            printf(" total energy, a.u.  = %25.16f\n", cc_opts->escf + ecorr);
            printf("\n");
            prev_ecorr = ecorr;
        }
