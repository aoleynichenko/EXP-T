...
void sector_0h1p_prop_linear();

void msprop_calculate_wavefunction_norms(
        int sect_h, int sect_p, int approximation,
        int nrep, struct mv_block *mv_blocks,
        double complex ***wf_norms_squared
);
...
void construct_property_matrix_slater(int sect_h, int sect_p, int nspinors, double complex *prp_spinor,
                                      double complex *prp_slater, slater_det_t *bra_dets, size_t n_det_bra,
                                      slater_det_t *ket_dets, size_t n_det_ket, double complex scalar_term,
                                      int n_diagrams, char **diagram_names)
{
    memset(prp_slater, 0, sizeof(double complex) * n_det_bra * n_det_ket);

    // add scalar term to the diagonal
    for (size_t i = 0; i < n_det_bra; i++) {
        for (size_t j = 0; j < n_det_ket; j++) {
            slater_det_t *bra = bra_dets + i;
            slater_det_t *ket = ket_dets + j;
            prp_slater[i * n_det_ket + j] = scalar_term * kronecker_delta(sect_h, sect_p, bra, ket);
        }
    }

    // loop over n-body diagrams
    for (int i = 0; i < n_diagrams; i++) {

        char *dg_name = diagram_names[i];
        diagram_t *dg_prop = diagram_stack_find(dg_name);
        if (dg_prop == NULL) {
            errquit("construct_property_matrix_slater(): diagram '%s' not found", dg_prop);
        }

        setup_slater(dg_prop, (matrix_getter_fun) diagram_get, sect_h, sect_p, sect_h, sect_p, rank(dg_name) / 2);

        for (size_t i = 0; i < n_det_bra; i++) {
            for (size_t j = 0; j < n_det_ket; j++) {
                slater_det_t *bra = bra_dets + i;
                slater_det_t *ket = ket_dets + j;
                prp_slater[i * n_det_ket + j] += slater(bra, ket);
            }
        }
    }
}
...
void model_space_property(cc_ms_prop_query_t *prop_query, int approximation)
{
    ...
    if (approximation == CC_PROPERTIES_APPROX_MODEL_SPACE) {
        printf("  ** Approximation: only model vectors are used\n");
    }
    else if (approximation == CC_PROPERTIES_APPROX_LINEAR) {
        printf("  ** Approximation: terms linear in cluster amplitudes\n");
    }
    printf("  **\n");
    printf("\n");
    ...
    /*
     * construct diagrams representing the property operator in the second-quantized form
     */
    request_sorting("op_hp", "hp", "00", "12");
    request_sorting("op_ph", "ph", "00", "12");
    request_sorting("op_pp", "pp", "00", "12");
    request_sorting("op_hh", "hh", "00", "12");
    sort_prop(nspinors, prop_spinor);

    /*
     * construct diagrams: (e^T)^\dagger O e^T
     */
    int n_diagrams = 0;
    char *prop_diagrams_names[10];
    double complex scalar_term_prop = 0.0 + 0.0 * I;

    if (sect_h >= 0 && sect_p >= 0) {
        printf(" construction of diagrams for the 0h0p sector\n");
        diagram_conjugate("t1c", "t1c+");
        diagram_conjugate("t2c", "t2c+");
        scalar_term_prop = sector_0h0p_prop(approximation);
        printf(" scalar (0h0p) contribution to property: %.8e %.8e\n", creal(scalar_term_prop),
               cimag(scalar_term_prop));
    }
    if (sect_h >= 0 && sect_p >= 1) {
        printf(" construction of diagrams for the 0h1p sector\n");
        diagram_conjugate("s1c", "s1c+");
        diagram_conjugate("s2c", "s2c+");
        sector_0h1p_prop("op_01", approximation);
        prop_diagrams_names[n_diagrams++] = strdup("op_01");
    }
    if (sect_h >= 0 && sect_p >= 2) {
        printf(" construction of diagrams for the 0h2p sector\n");
        diagram_conjugate("x2c", "x2c+");
        sector_0h2p_prop("op_02", approximation);
        prop_diagrams_names[n_diagrams++] = strdup("op_02");
    }
    printf("\n");


    /*
     * calculate norms of wavefunctions
     */
    printf(" norms of wavefunctions\n\n");
    double complex **wf_norms = NULL;

    msprop_calculate_wavefunction_norms(sect_h, sect_p, approximation, nrep, mv_blocks, &wf_norms);

    for (int irep = 0; irep < nrep; irep++) {
        struct mv_block *block = &mv_blocks[irep];

        printf(" irrep=%-10s    re(norm^2)  im(norm^2)     norm\n", block->rep_name);
        for (size_t i = 0; i < block->nroots; i++) {
            double complex norm2 = wf_norms[irep][i];
            double norm = sqrt(creal(norm2));
            printf(" %3d%12.2f   %12.8f%12.8f   %12.8f\n",
                   i + 1, block->energy_cm[i], creal(norm2), cimag(norm2), norm);
        }
        printf("\n");
    }
     ...
     */
    printf(" property matrix elements\n\n");
    ...
    for (int irep1 = 0; irep1 < nrep; irep1++) {
        for (int irep2 = 0; irep2 < nrep; irep2++) {
            ...
            double complex *prop_slater = x_zeros(CC_COMPLEX, block1->ms_size, block2->ms_size);
            double complex *prop = x_zeros(CC_COMPLEX, block1->nroots, block2->nroots);

            // construct property matrix in the basis of Slater determinants
            construct_property_matrix_slater(sect_h, sect_p, nspinors, prop_spinor, prop_slater,
                                             block1->dets, block1->ms_size, block2->dets, block2->ms_size,
                                             scalar_term_prop,
                                             n_diagrams, prop_diagrams_names);
            ...
            // each matrix element should be divided by normalization factors
            ...
            for (size_t i = 0; i < nroots_i; i++) {
                for (size_t f = 0; f < nroots_f; f++) {
                    ...
                    double complex norm_i_squared = wf_norms[irep1][i];
                    double complex norm_f_squared = wf_norms[irep2][f];
                    double norm_i = sqrt(creal(norm_i_squared));
                    double norm_f = sqrt(creal(norm_f_squared));

                    //prop[i * nroots_f + f] = prop_if / (norm_i * norm_f);
                }
            }
...
/**
 * calculates squared norms <psi|psi> of FS-CC wavefunctions
 * (in non-trivial Fock space sectors).
 *
 * @param sect_h
 * @param sect_p
 * @param approximation
...
)
{
    ...
    /*
     * construct diagrams: (e^T)^\dagger e^T
     */
    int n_diagrams = 0;
    char *norm_diagrams_names[10];
    double complex scalar_term_norm = 0.0 + 0.0 * I;

    if (sect_h >= 0 && sect_p >= 0) {
        diagram_conjugate("t1c", "t1c+");
        diagram_conjugate("t2c", "t2c+");
        scalar_term_norm = sector_0h0p_norm(approximation);
    }
    if (sect_h >= 0 && sect_p >= 1) {
        diagram_conjugate("s1c", "s1c+");
        diagram_conjugate("s2c", "s2c+");
        sector_0h1p_norm("norm_01", approximation);
        norm_diagrams_names[n_diagrams++] = strdup("norm_01");
    }
    if (sect_h >= 0 && sect_p >= 2) {
        diagram_conjugate("x2c", "x2c+");
        sector_0h2p_prop("op_02", approximation);
        sector_0h2p_norm("norm_02", approximation);
        norm_diagrams_names[n_diagrams++] = strdup("norm_02");
    }

    /*
     * allocate memory
     */
    *wf_norms_squared = (double complex **) cc_calloc(nrep, sizeof(double complex *));
    for (int irep = 0; irep < nrep; irep++) {
        struct mv_block *block = mv_blocks + irep;
        (*wf_norms_squared)[irep] = (double complex *) cc_calloc(block->nroots, sizeof(double complex));
    }

    /*
     * calculate norms of wavefunction:
     * 1. transformation of diagrams to the basis of Slater determinants
     * 2. transformation to the basis of model vectors
     */
    for (int irep = 0; irep < nrep; irep++) {
        struct mv_block *block = &mv_blocks[irep];

        double complex *norm_slater_ket = x_zeros(CC_COMPLEX, block->ms_size, block->ms_size);
        double complex *norm_ket = x_zeros(CC_COMPLEX, block->nroots, block->nroots);

        construct_property_matrix_slater(sect_h, sect_p, 0, NULL, norm_slater_ket,
                                         block->dets, block->ms_size, block->dets, block->ms_size, scalar_term_norm,
                                         n_diagrams, norm_diagrams_names);

        msprop_transform_slater_to_model(block->nroots, block->nroots,
                                         block->ms_size, block->ms_size,
                                         block->vl, block->vr,
                                         norm_slater_ket, norm_ket);

        for (size_t i = 0; i < block->nroots; i++) {
            double complex norm2 = norm_ket[i * block->nroots + i];
            (*wf_norms_squared)[irep][i] = norm2;
        }

        cc_free(norm_slater_ket);
        cc_free(norm_ket);
    }
