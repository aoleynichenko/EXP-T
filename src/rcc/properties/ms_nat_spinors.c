...
void write_model_natural_spinors_formatted(
...
void print_model_space_natural_spinor(FILE *out, int n_active, int *active_spinors, double complex *nat_spinor_coefs,
                                      double thresh);
...
void construct_model_space_natural_spinors(int sect_h, int sect_p, int irrep, int state)
{
    ...
    for (int i = 0; i < n_active; i++) {
        ...
        print_model_space_natural_spinor(stdout, n_active, active_spinors, nat_spinors_right + n_active * i,
                                         COEF_THRESH);
    }
    ...
    write_model_natural_spinors_formatted(natorb_file_name, sect_h, sect_p, n_active, nat_occ_re, nat_spinors_right,
                                          OCC_THRESH);
...
)
{
    ...
    for (int i = 0; i < n_active; i++) {
        ...
        printf("    from:\n");
        print_model_space_natural_spinor(stdout, n_active, active_spinors, nat_spinor_from + n_active * i, COEF_THRESH);
        printf("    to:\n");
        print_model_space_natural_spinor(stdout, n_active, active_spinors, nat_spinor_to + n_active * i, COEF_THRESH);
    }
    ...
    str_replace(natorb_file_name, '/', '|');  // for some irrep names with '/' in order to prevent bugs
    write_model_natural_spinors_formatted(natorb_file_name, sect_h, sect_p, n_active, lambda, nat_spinor_from,
                                          OCC_THRESH);

    sprintf(natorb_file_name, "NTO_%dh%dp_%s_%d_%s_%d_FINAL.dat",
            sect_h, sect_p, rep_name1, state1 + 1, rep_name2, state2 + 1);
    str_replace(natorb_file_name, '/', '|');
    write_model_natural_spinors_formatted(natorb_file_name, sect_h, sect_p, n_active, lambda, nat_spinor_to,
                                          OCC_THRESH);
