...
void update(char *dg1_name, double factor, char *dg2_name)
{
    ...
    for (int i = 0; i < dg1->rank; i++) {
        if (dg1->valence[i] != dg2->valence[i]) {
            summary(dg1_name);
            summary(dg2_name);
            errquit("update(): 'valence' strings must coincide");
        }
        if (dg1->qparts[i] != dg2->qparts[i]) {
            summary(dg1_name);
            summary(dg2_name);
    ...
    /*if (dg1->only_unique != dg2->only_unique) {
        restore_diagram(dg1);
        restore_diagram(dg2);
    }*/
