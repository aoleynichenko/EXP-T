...
double get_shift_value(int sect_h, int sect_p, int rank, int *indices)
{
     ...
     */
    if (cc_opts->do_orbshift == 0 && cc_opts->do_orbshift_0h0p == 0) {
        double shift = cc_opts->shifts[sect_h][sect_p].shifts[rank/2 - 1];
        return shift;
    }

    /*
     * orbital shifts in non-trivial sectors, including IH-FSCC-like case
    ...
    /*
     * orbital shifts in the 0h0p sector;
     * one fixed value for all types of amplitudes.
     * (experimental code)
     */
    else if (cc_opts->do_orbshift_0h0p == 1 && sect_0h0p) {
        int n_active = count_active_indices(rank, indices);
        return n_active * cc_opts->orbshift / 2.0;
    }
    /*
     * orbital shifts in the 0h0p sector;
     * shift values for each amplitude type are specified by hands (in the input file).
     * (experimental code)
     */
    else if (cc_opts->do_orbshift_0h0p == 2 && sect_0h0p) {
        int n_active = count_active_indices(rank, indices);
        return (n_active > 0) ? cc_opts->orbshift_0h0p[2 + n_active - 1] : 0.0;
    }
...
inline int count_active_indices(int rank, int *indices)
{
    int n_active = 0;

    for (int i = 0; i < rank; i++) {
        n_active += is_active(indices[i]);
    }
...
inline int count_active_intermediate_indices(int rank, int *indices)
{
    int n_active_interm = 0;

    for (int i = 0; i < rank; i++) {
        n_active_interm += is_active_intermediate(indices[i]);
    }
