...
int sector03(cc_options_t *opts)
{
    ...
    int triples = (opts->cc_model < CC_MODEL_CCSDT_1A) ? 0 : 1;
    ...
    if (cc_opts->cc_model < CC_MODEL_CCSDT_2) {
        ...
        for (it = 1; it <= opts->maxiter; it++) {
            ...
            /*if (cc_opts->cc_model == CC_MODEL_CCSDT_2 || cc_opts->cc_model == CC_MODEL_CCSDT_3) {
                // calculate Veff = (V \Omega_SDT)_cl
                // not optimal code, just for accuracy benchmarks
                int model = cc_opts->cc_model;
                cc_opts->cc_model = CC_MODEL_CCSDT;
                //rename("z3nw", "z3nw_SDT-n");
                dg_stack_pos_t pos = get_stack_pos();
                calc_Z3();
                closed("z3nw", "veff03");
                restore_stack_pos(pos);
                //rename("z3nw_SDT-n", "z3nw");
                cc_opts->cc_model = model;
            }*/

            apply_selections(0, 3, "z3nw");

            if (opts->do_relax) {
                remove_core_correlation("z3nw");
            }
            ...
            if (cc_opts->restrict_triples && triples) {
                restrict_triples("z3nw", cc_opts->restrict_triples_e1, cc_opts->restrict_triples_e2);
            }
    ...
    diag_heff(0, 3, "veff01", "veff02", "veff03");
