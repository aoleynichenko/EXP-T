...
inline int is_active_main(int idx)
{
    return is_active(idx) && ((spinor_info[idx].space_flags & CC_FLAG_MAIN) ? 1 : 0);
}


inline int is_active_intermediate(int idx)
{
    return is_active(idx) && !((spinor_info[idx].space_flags & CC_FLAG_MAIN) ? 1 : 0);
}


inline int set_main(int idx)
{
    spinor_info[idx].space_flags |= CC_FLAG_MAIN;
}
...
void setup_active_space(cc_options_t *cc_opts)
{
    ...
    if (cc_opts->actsp_defined == CC_ACT_SPACE_SPEC_ENERGY) {
        setup_active_space_by_energy(cc_opts->actsp_min, cc_opts->actsp_max, CC_FLAG_ACTIVE);
    }
